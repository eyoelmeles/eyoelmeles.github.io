...
One of the most important tool that JavaScript lacks is *Types*. that is why we have TypeScript and it got its name from it.
...
- Objects (pretty much everything else is an object in JavaScript)
    ...
    1. ### Interface
        ...
        The only main differnce between interface and type is that we can *extend interfaces*, but not types.

        TypeScript assumes *duck typing* for an interface.
    ...
    2. ### Type
        ...
        in one group, this is where type comes in. the other benefit to using type is *naming*. we get even name unions.
...
there are also *literal types* which takes the value of the variable as a type.
example could be
...
* `as`
  ...
  is a TypeScript syntax not a type but it means use this variable/function as a type we provided, meaning telling the let variables to be of a specific type when they get assigned a value later. as can be viewed as *casting*.
